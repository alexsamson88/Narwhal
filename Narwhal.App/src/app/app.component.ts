...
export class AppComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.map.on('load', function() {
            ...
            fetch('/api/tracking/get?from=2018-04-23&to=2018-04-25')
                .then(response => response.json())
                .then(data => {
        ...
        this.map.on('load', function() {
            ...
            this.addSource('vessel-intersection', {
                type: 'geojson',
                data: {
                    type: 'FeatureCollection',
                    features: []
            ...
            this.addLayer({
                id: 'intersection-point-layer',
                type: 'circle',
                source: 'vessel-intersection',
                paint: {
                    'circle-radius': 10,
                    'circle-color': "red",
                    'circle-stroke-color': "#000000",
            ...
            fetch('/api/IntersectionWarnings/Get?from=2018-04-23&to=2018-04-25')
                .then(response => response.json())
                .then(data => {
                    console.log(data);
                    let intersections = [];
                    ...
                    for (const intersection of data) {
                        ...
                        intersections.push({
                            "type" : "Feature",
                            "properties": {
                                "vessel1IntersectionCrossingDate":  intersection.vessel1IntersectionCrossingDate,
                                "vessel2IntersectionCrossingDate":  intersection.vessel2IntersectionCrossingDate,
                                "vessel1":  intersection.vessel1,
                                "vessel2":  intersection.vessel2
                            },
                            "geometry" : {
                                "type" : "Point",
                                "coordinates" : [intersection.intersectionPoint.longitude, intersection.intersectionPoint.latitude ]
                            }
                        });
                    }
                    ...
                    console.log(geoJson);
                    this.getSource('vessel-intersection').setData(geoJson);
                });
            ...
            this.on('mouseenter', 'intersection-point-layer', function (e) {
                // Change the cursor style as a UI indicator.
                this.getCanvas().style.cursor = 'pointer';
                console.log(e);
                var html = `<p>Vessel: ${e.features[0].properties.vessel1} ${e.features[0].properties.vessel1IntersectionCrossingDate}</p>`;
                html += `<p>Vessel: ${e.features[0].properties.vessel2} ${e.features[0].properties.vessel2IntersectionCrossingDate}</p>`;
                
                // Populate the popup and set its coordinates
                // based on the feature found.
                popup.setLngLat(e.lngLat).setHTML(html).addTo(this);
            });
                
            this.on('mouseleave', 'intersection-point-layer', function () {
                this.getCanvas().style.cursor = '';
                popup.remove();
            });
        });
